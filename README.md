...
Проверить, можно ли в Москве (плотная застройка, радиошумы) принимать и декодировать сигналы станции точного времени [РБУ (66,666 кГц)](https://ru.wikipedia.org/wiki/RBU), используя
SDR-приёмник Малахит DSP3 и самодельную ферритовую антенну, с минимальными
аппаратными доработками.

Спойлер: да, можно. Дата, день недели, час и минута восстанавливаются из эфира полностью (см. раздел «Результаты»).


## Что такое РБУ

[РБУ](https://ru.wikipedia.org/wiki/RBU) — российская станция эталонной частоты и точного времени, работает на 66,666 кГц, передатчик находится в Подмосковье, мощность 10 или 50 кВт (источники дают разную информацию). Помимо эталонной несущей, она непрерывно передаёт цифровой код текущего времени (дата, день недели, час, минута), который можно принять и декодировать даже на простой приёмник на боковых частотах ±100 и ±312,5 Гц. Протокол простой и описан.
...
* **Антенна:** самодельная ферритовая (см. Рис. 1). Стержень — китайский феррит с Ozon, 200×10 мм. Катушка намотана обычным многожильным проводом ~0,75 мм по изоляции (литцендрата/ПЭВ под рукой не было), длина намотки ~130 мм, намотал сколько было около 5,5 метров. Катушка связи — 3 витка монтажного провода. Индуктивность такой катушки ≈ 1,83 мГн, резонансная ёмкость на 66,666 кГц ≈ 3,1 нФ. Настройка в резонанс — подстроечным конденсатором по спектроанализатору RSA3015N и генератору с перестраиваемой частотой 60-70 кГц.
...
день, час, минута) разложены по битам с привязкой к конкретной секунде минуты, плюс есть контроль чётности **P1–P8** (подробное описание см в [1]).
...
Эталонный спектр с анализатора RSA3015N (см Рис. 2): несущая 66,666 кГц (−106,8 дБм) и боковые ±100 Гц.  

![RSA3015N_RBU](img/signal_rbu.png)
Рис. 2. Спектр РБУ на RSA3015N: несущая и боковые ±100 Гц

На экране Малахита (смРис. 3) несущая и боковые тоже видны, но рядом — сильная помеха.

![Malahit_RBU](img/malahit_rbu.png)
Рис. 3. Экран Малахита: несущая, боковые и помеха рядом
